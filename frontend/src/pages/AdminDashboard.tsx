...
import { BarChart3, BookOpen, Edit3, Eye, FolderKanban, Heart, LogOut, Mail, MessageSquare, Search, Settings, Star, Trash2, Video } from "lucide-react";
import { useQuery, useQueryClient } from "@tanstack/react-query";
import SEO from "@/components/SEO";
import { api, BlogPayload, BlogPost } from "@/lib/api";
...
const AdminDashboard = () => {
  ...
  const dashboard = useQuery({ queryKey: ["adminDashboard", token], queryFn: () => api.getAdminDashboard(token), enabled: Boolean(token), retry: false });
  const adminBlogs = useQuery({ queryKey: ["adminBlogs", token, blogSearch, blogStatus], queryFn: () => api.getAdminBlogPosts(token, { search: blogSearch, status: blogStatus }), enabled: Boolean(token), retry: false });
  ...
  const handleLogin = async (event: FormEvent) => {
    ...
    try {
      ...
      if (result.data.user.role !== "admin") return setError("This account is not an admin account. Check ADMIN_EMAIL and ADMIN_PASSWORD on Render.");
      localStorage.setItem("devwithsunil_admin_token", result.data.token); setToken(result.data.token); setSuccess("Admin login successful.");
    } catch (err) { setError(err instanceof Error ? err.message : "Login failed"); }
  };

  const logout = () => { localStorage.removeItem("devwithsunil_admin_token"); setToken(""); setPassword(""); queryClient.clear(); };

  const saveBlog = async (event: FormEvent) => {
    event.preventDefault(); setError(""); setSuccess("");
    try {
      const payload = { ...blog, slug: makeSlug(blog.slug), tags: blog.tags || [], cover_image: blog.cover_image || null, seo_title: blog.seo_title || null, seo_description: blog.seo_description || null, scheduled_at: blog.scheduled_at || null };
      if (editingBlogId) await api.updateBlogPost(token, editingBlogId, payload);
      else await api.createBlogPost(token, payload);
      setBlog(baseBlog()); setEditingBlogId(null); setSuccess(editingBlogId ? "Blog updated successfully." : "Blog saved successfully."); refreshAdminData();
    } catch (err) { setError(err instanceof Error ? err.message : "Failed to save blog post"); }
  };
  ...
  const deleteBlog = async (id?: number) => {
    ...
    try { await api.deleteBlogPost(token, id); setSuccess("Blog deleted successfully."); refreshAdminData(); }
    catch (err) { setError(err instanceof Error ? err.message : "Failed to delete blog"); }
  };

  const createVideo = async (event: FormEvent) => {
    event.preventDefault(); setError(""); setSuccess("");
    try { await api.createYoutubeVideo(token, { ...video, published_at: video.published_at || undefined }); setVideo(emptyVideo); setSuccess("YouTube video saved successfully."); refreshAdminData(); }
    catch (err) { setError(err instanceof Error ? err.message : "Failed to create video"); }
  };

  const createProject = async (event: FormEvent) => {
    event.preventDefault(); setError(""); setSuccess("");
    try {
      await api.createProject(token, { title: project.title, slug: project.slug, description: project.description, tech_stack: stringToTags(project.tech_stack), github_url: project.github_url || undefined, live_url: project.live_url || undefined, featured: project.featured });
      setProject(emptyProject); setSuccess("Project saved successfully."); refreshAdminData();
    ...
    <div className="min-h-screen bg-background px-4 py-10">
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="font-mono text-sm text-primary">Admin workspace</p><h1 className="mt-2 font-mono text-3xl font-bold">DevWithSunil Dashboard</h1><p className="mt-2 max-w-2xl text-sm text-muted-foreground">Manage blogs, YouTube videos, projects, subscribers, contact messages, SEO, and scheduled publishing.</p></div>
          <div className="flex gap-3"><Link className="rounded-lg border border-border px-4 py-2 text-sm" to="/">View site</Link>{token ? <button onClick={logout} className="inline-flex items-center gap-2 rounded-lg bg-destructive px-4 py-2 text-sm font-semibold text-destructive-foreground"><LogOut className="h-4 w-4" /> Logout</button> : null}</div>
        </div>
        ...
        {!token ? (
          <form onSubmit={handleLogin} className="mx-auto max-w-md rounded-2xl border border-border/60 bg-card p-6 shadow-sm">
            <h2 className="font-mono text-xl font-bold">Admin Login</h2><p className="mt-2 text-sm text-muted-foreground">Use the admin email and password configured in Render environment variables.</p>
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">{statCards.map((card) => <div key={card.label} className="rounded-2xl border border-border/60 bg-card p-4 shadow-sm"><card.icon className="h-5 w-5 text-primary" /><p className="mt-3 text-xs uppercase tracking-wide text-muted-foreground">{card.label}</p><p className="mt-1 font-mono text-2xl font-bold">{card.value}</p></div>)}</div>

            <form onSubmit={saveBlog} className="mt-8 rounded-2xl border border-border/60 bg-card p-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><h2 className="font-mono text-xl font-semibold">{editingBlogId ? "Edit Blog" : "Create Blog"}</h2><button type="button" onClick={() => setShowPreview(!showPreview)} className="rounded-lg border border-border px-4 py-2 text-sm">{showPreview ? "Hide Preview" : "Show Preview"}</button></div>
              <div className="mt-5 grid gap-4 lg:grid-cols-2">
                <input className="rounded-lg border border-border bg-background px-3 py-2 text-sm" placeholder="Title" value={blog.title} onChange={(e) => setBlog({ ...blog, title: e.target.value, slug: blog.slug || makeSlug(e.target.value), seo_title: blog.seo_title || e.target.value.slice(0, 70) })} required />
                <input className="rounded-lg border border-border bg-background px-3 py-2 text-sm" placeholder="slug-example" value={blog.slug} onChange={(e) => setBlog({ ...blog, slug: makeSlug(e.target.value) })} required />
                <input className="rounded-lg border border-border bg-background px-3 py-2 text-sm" placeholder="Category" value={blog.tag} onChange={(e) => setBlog({ ...blog, tag: e.target.value })} required />
                ...
                <input className="lg:col-span-2 rounded-lg border border-border bg-background px-3 py-2 text-sm" placeholder="Thumbnail / cover image URL" value={blog.cover_image || ""} onChange={(e) => setBlog({ ...blog, cover_image: e.target.value })} />
                ...
                <textarea className="lg:col-span-2 min-h-20 rounded-lg border border-border bg-background px-3 py-2 text-sm" placeholder="Excerpt" value={blog.excerpt} onChange={(e) => setBlog({ ...blog, excerpt: e.target.value, seo_description: blog.seo_description || e.target.value.slice(0, 170) })} required />
                <textarea className="lg:col-span-2 min-h-72 rounded-lg border border-border bg-background px-3 py-2 font-mono text-sm" placeholder="Rich text / Markdown content. Use # headings, **bold**, lists, and links." value={blog.content} onChange={(e) => setBlog({ ...blog, content: e.target.value })} required />
              </div>
